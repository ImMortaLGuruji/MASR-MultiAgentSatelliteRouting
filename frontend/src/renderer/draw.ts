...
export function resetDrawState() {
  stars.length = 0;
  visualSatellites.clear();
  packetProgress.clear();
}

export function initStars(width: number, height: number, force = false) {
...
export function project(
  ...
  const px =
    center.cx +
    (x * Math.cos(rotationOffset) - z * Math.sin(rotationOffset)) * scale;
...
export function drawOrbits(
  ...
  for (const alt of Array.from(altitudeSet).sort(
...
export function drawSatellites(
  ...
  for (const sat of snapshot.satellites) {
    ...
    const bufferUsage =
      (sat.packet_queue?.length || 0) / Math.max(sat.buffer_capacity || 1, 1);
    const color =
      bufferUsage > 0.8 ? "#ef4444" : bufferUsage > 0.4 ? "#f59e0b" : "#f8fafc";
...
export function drawPackets(
  ...
  const satById = new Map(
    snapshot.satellites.map((sat) => [sat.satellite_id, sat]),
  );
  const activePacketIds = new Set(
    snapshot.packets.map((packet) => packet.packet_id),
  );

  for (const packet of snapshot.packets) {
    const history = packet.route_history || [];
    const fromId =
      history.length > 1 ? history[history.length - 2] : packet.current_holder;
